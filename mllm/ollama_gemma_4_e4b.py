"""
Ollama **gemma4:e4b** (text or image + text → text) via the local Ollama HTTP API.

Requires a running ``ollama serve`` and ``ollama pull gemma4:e4b`` (or the tag set in YAML / env).

Environment:

- ``OLLAMA_HOST`` — API base URL (default from ollama-python: ``http://127.0.0.1:11434``).
- ``OLLAMA_GEMMA4_E4B_MODEL`` — model tag (default ``gemma4:e4b``).

Repo ``config/model.yaml`` → ``ollama.model_e4b`` is mirrored into ``OLLAMA_GEMMA4_E4B_MODEL`` via
:func:`util.repo_config.ensure_repo_config_applied` when the env var is unset.
"""
...
_ENV_MODEL = "OLLAMA_GEMMA4_E4B_MODEL"
_DEFAULT_MODEL = "gemma4:e4b"


def default_model() -> str:
    """Resolved model tag: env ``OLLAMA_GEMMA4_E4B_MODEL`` or ``gemma4:e4b``."""
